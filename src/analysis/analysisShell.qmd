...
## MEL including fuel and wet mass


```{r}
df_mel_wet <- df_mel_after %>%
  add_row(c_id="C.03", name="orbiter-spacecraft-fuel", mass=round(df_parameters$m_fuel,0), componenttype="fuel") %>%
  add_row(c_id="C.04", name="orbiter-spacecraft-wet", mass=round(df_parameters$m_wet,0), componenttype="system") 
...
df_table$name[df_table$componenttype == "subsystem"] <- paste0("\s\s\s", df_table$name[df_table$componenttype == "subsystem"])
