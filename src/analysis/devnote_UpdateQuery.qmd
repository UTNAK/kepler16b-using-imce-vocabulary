---
title: "Dev-Note for SPARQL Update"
date: now
...
### query for updating parameters

this does not work well. so we need to separate deleting and inserting.
...
query_string_update <- str_replace(query_string_update, "\\$UPDATEVALUE", update_value)
query_string_update <- str_replace(query_string_update, "\\$UPDATEIRI", update_iri)
query_string_update <- str_replace(query_string_update, "\\$GRAPHIRI", graph_iri)

cat(query_string_update)
```

```{r}
query_string_update_tbd <- '
PREFIX owl: <http://www.w3.org/2002/07/owl#>
PREFIX base:        <http://imce.jpl.nasa.gov/foundation/base#>
PREFIX mission:        <http://imce.jpl.nasa.gov/foundation/mission#>
PREFIX analysis:        <http://imce.jpl.nasa.gov/foundation/analysis#>
...
query_string_update_tbd <- str_replace(query_string_update_tbd, "\\$UPDATEVALUE", update_value)
query_string_update_tbd <- str_replace(query_string_update_tbd, "\\$UPDATEIRI", update_iri)
query_string_update_tbd <- str_replace(query_string_update_tbd, "\\$GRAPHIRI", graph_iri)
query_string_update_tbd <- str_replace(query_string_update_tbd, "\\$GRAPHIRI", graph_iri)

cat(query_string_update_tbd)
```

### Get Intended State Value using affects relationship
```
PREFIX base:        <http://imce.jpl.nasa.gov/foundation/base#>
PREFIX mission:        <http://imce.jpl.nasa.gov/foundation/mission#>
PREFIX analysis:        <http://imce.jpl.nasa.gov/foundation/analysis#>
...
SELECT ?iri ?statevariable ?value
WHERE {
...
### Send Query
This doesn't work. Needs to be fixed at [issue #11](https://github.com/UTNAK/kepler16b-using-imce-vocabulary/issues/11) .
duplicated data. delete doesn't work well.
lets make separated.
query_string_update_tbd
...
```{r}
endpoint_url <- "http://localhost:3030/tutorial2/sparql"
send_query(endpoint_url,query_param_string)


endpoint_url <- "http://localhost:3030/tutorial2/sparql"
send_query(endpoint_url, query_string_get_sv)


endpoint_url <- "http://localhost:3030/tutorial2-tdb/sparql"
send_query(endpoint_url, query_string_get_sv)


# df_query <- send_query(endpoint_url, query_string_update)
# httpResponse <- httr::POST(url = endpoint_url, query = list(query = query_string_update))
```

## delete and insert test
...
query_string_delete_tdb <- str_replace(query_string_delete_tdb, "\\$UPDATEVALUE", update_value)
...
cat(query_string_insert_tdb)
```

### sending update query

what is endpoint url so far?
...
```curl
curl http://localhost:3030/tutorial2-tdb/update --data update=PREFIX%20owl%3A%20%3Chttp%3A%2F%2Fwww.w3.org%2F2002%2F07%2Fowl%23%3E%0APREFIX%20base%3A%20%20%20%20%20%20%20%20%3Chttp%3A%2F%2Fimce.jpl.nasa.gov%2Ffoundation%2Fbase%23%3E%0APREFIX%20mission%3A%20%20%20%20%20%20%20%20%3Chttp%3A%2F%2Fimce.jpl.nasa.gov%2Ffoundation%2Fmission%23%3E%0APREFIX%20analysis%3A%20%20%20%20%20%20%20%20%3Chttp%3A%2F%2Fimce.jpl.nasa.gov%2Ffoundation%2Fanalysis%23%3E%0APREFIX%20sa%3A%20%20%20%20%20%20%20%20%3Chttp%3A%2F%2Fexample.com%2Ftutorial2%2Fvocabulary%2Fstateanalysis%23%3E%0A%0Adelete%20%7B%0A%20%20graph%20%3Chttp%3A%2F%2Fexample.com%2Ftutorial2%2Fdescription%2Fstatedictionary%3E%20%7B%0A%20%20%20%20%3FanalysisTarget%20sa%3AhasStateValue%20%3FbeforeValue%20.%0A%20%20%7D%0A%7D%0A%0Awhere%7B%0A%20%20%20%20BIND%28%3Chttp%3A%2F%2Fexample.com%2Ftutorial2%2Fdescription%2Fstatedictionary%23orbiter-spacecraft.isp%3E%20AS%20%3FanalysisTarget%29%0A%0A%20%20%09%09%3FanalysisTarget%20a%20owl%3ANamedIndividual%20%3B%0A%20%20%20%20%20%20%09%09%09sa%3AhasStateValue%20%3FbeforeValue%20.%0A%7D -X POST
```

this is what we need for update query
finally it works

```{r}
# URLとデータを指定
url <- "http://localhost:3030/tutorial2-tdb/"
data <- list(update = query_string_delete_tdb)  # ここに実際のデータを入力

# POSTリクエストを送信
response <- httr::POST(url, body = data, encode = "form")
httr::content(response, as = "text", encoding = "UTF-8")
```

test this using a package

```{r}
send_update(endpoint_url = "http://localhost:3030/tutorial2-tdb/", query_string_delete_tdb)
send_update(endpoint_url = "http://localhost:3030/tutorial2-tdb/", query_string_insert_tdb)
